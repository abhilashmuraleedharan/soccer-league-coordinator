 ...
 * b) Every team's players average height should not be more than 1.5 inches of each other's
 */
...
// Struct Email to model email to be sent to players' guardians
struct Email {
    let to: String
...
class Team {
    ...
    func prepGuardianEmails() {
        ...
        var content: String
        for player in players {
            content = "Hi, \(player.guardian). Your child \(player.name) from Team \(name) will attend their first team practice on \(practiceDateTime)"
...
class League {
    ...
    func information() {
        ...
        if teams.count > 0 {
            ...
            print("No players and teams added to the league yet!")
        }
    }
    ...
    /// Instance method to sort players into teams
    func sortPlayers(intoTeams teams: [String]) {
        ...
        // Put all experienced and inexperienced players in separate collections
        for player in players {
         ...
         * c) Pick players from these sorted lists while adding to a team so that
    ...
    func assignToTeams(schedules: [String] ) {
        if schedules.count != teamsCount {
            print("Missing Information. Please pass practice schedules for all teams")
            return
        }
        var i: Int = 0
        for team in teams {
            team.set(practiceDateTime: schedules[i])
            i += 1
        }
    }
...
/* 
 * As a soccer league coordinator use the above defined custom objects to create your own Soccer League and Soccer Teams
 */

// Create a Soccer League
...
let player2 = Player(name: "Jill Tanner", height: 36.0, experience: true, guardian: "Clara Tanner",
                     guardianEmailId: "clara.tanner@gmail.com")
let player3 = Player(name: "Bill Bon", height: 43.0, experience: true, guardian: "Sara and Jenny Bon", guardianEmailId: "sara@gmail.com")
let player4 = Player(name: "Eva Gordon", height: 45.0, experience: false, guardian: "Wendy and Mike Gordon",
                     guardianEmailId: "wendy@gmail.com")
let player5 = Player(name: "Matt Gill", height: 40.0, experience: false, guardian: "Charles and Sylvia Gill",
                     guardianEmailId: "charles@gmail.com")
let player6 = Player(name: "Kimmy Stein", height: 41.0, experience: false, guardian: "Bill and Hillary Stein",
                     guardianEmailId: "bill@gmail.com")
...
let player9 = Player(name: "Suzane Greenberg", height: 44.0, experience: true, guardian: "Henrietta Dumas",
                     guardianEmailId: "henrietta@gmail.com")
let player10 = Player(name: "Sal Dali", height: 41.0, experience: false, guardian: "Gala Dali", guardianEmailId: "gala.dali@gmail.com")
let player11 = Player(name: "Joe Kavalier", height: 39.0, experience: false, guardian: "Sam and Elaine Kavalier",
                      guardianEmailId: "sam@gmail.com")
let player12 = Player(name: "Ben Finkelstein", height: 44.0, experience: false, guardian: "Aaron and Jill Finkelstein",
                      guardianEmailId: "aaron@gmail.com")
...
let player14 = Player(name: "Chloe Alaska", height: 47.0, experience: false, guardian: "David and Jamie Alaska",
                      guardianEmailId: "david@gmail.com")
...
let player16 = Player(name: "Phillip Helm", height: 44.0, experience: true, guardian: "Thomas Helm and Eva Jones",
                      guardianEmailId: "thomas@gmail.com")
let player17 = Player(name: "Les Clay", height: 42.0, experience: true, guardian: "Wynonna Brown", guardianEmailId: "wynonna@gmail.com")
let player18 = Player(name: "Herschel Krustofski", height: 45.0, experience: true, guardian: "Hyman and Rachel Krustofski",
                      guardianEmailId: "hyman@gmail.com")

// View any player's information using the information() instance method as illustrated below
print("Player 15 details :")
print("----------------------")
player15.information()
print("")

// Add the Players to the League
let players = [ player1, player2, player3, player4, player5, player6, player7, player8, player9, player10, player11, player12, player13,
                player14, player15, player16, player17, player18 ]
for leaguePlayer in players {
    league.add(player: leaguePlayer)
}

// Declare the Soccer Teams
let leagueTeams = ["Dragons", "Sharks", "Raptors"] // Since while creating soccer league object, teams count specified was only 3

// Distribute players across the soccer teams
league.sortPlayers(intoTeams: leagueTeams)

// Store team details in respective team variables
let teamDragons = league.teams[0]
let teamSharks  = league.teams[1]
let teamRaptors = league.teams[2]

// View each team's detail using the information() instance method
teamDragons.information()
teamSharks.information()
teamRaptors.information()

// View average height of players of each team using the averageHeightOfPlayers() instance method
...
// Give each team their practice schedules
let teamsPracticeTimes = [
                          "Team Dragons": "March 17, 1pm",
                          "Team Sharks": "March 17, 3pm",
                          "Team Raptors": "March 18, 1pm"
                         ]
var practiceSchedules = [String]()
for ( _ , schedule) in teamsPracticeTimes {
    practiceSchedules.append(schedule)
}
league.assignToTeams(schedules: practiceSchedules)

// Prepare and send emails to guardians of all league players
// informing them of their ward's team and practice schedules
league.informGuardians()

// Consolidate all emails to a constant for future use if any
